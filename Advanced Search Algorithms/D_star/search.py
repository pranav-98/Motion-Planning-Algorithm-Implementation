...
class DStar:
    ...
    def process_state(self):
        ...
        if k_old<current_Node.h:
            for n in neighbors:
                if n.h<=k_old and current_Node.h>(n.h+self.cost(current_Node,n)):
                    current_Node.h=n.h+self.cost(current_Node,n)
                    current_Node.parent=n

        if k_old== current_Node.h:
            for n in neighbors:
                if n.tag=="NEW" or n.parent==current_Node and n.h!= current_Node.h+self.cost(current_Node,n) or n.parent!=current_Node and n.h>current_Node.h+self.cost(current_Node,n):
                    n.parent=current_Node
                    self.insert(n,current_Node.h +self.cost(n,current_Node))
        else:
            for n in neighbors:
                if n.tag=="NEW" or n.parent==current_Node and n.h!=current_Node.h+self.cost(n,current_Node):
                    n.parent=current_Node
                    self.insert(n, current_Node.h+self.cost(n,current_Node))
                else:
                    if n.parent!=current_Node and n.h>current_Node.h+self.cost(current_Node,n):
                        self.insert(current_Node,current_Node.h)
                    else:
                        if n.parent!=current_Node and current_Node.h>n.h+self.cost(current_Node,n) and n.tag=="CLOSED" and n.h>k_old:
                            self.insert(n,n.h)
    ...
    def repair_replan(self, node):
        ...
        while True:
            ...
            if len(self.open)==0 or k_min>=node.h:
                break
        #### TODO END ####
    ...
    def prepare_repair(self, node):
        ...
        for n in self.get_neighbors(node):
            if n.is_dy_obs==True and n.is_obs==False:
                n.is_obs=True
                for nn in self.get_neighbors(n):
                    self.modify_cost(n,nn)
    ...
    def run(self):
        ...
        while True:
            k_min=self.process_state()
            if len(self.open)==0 or current.tag=="CLOSED":
                break
