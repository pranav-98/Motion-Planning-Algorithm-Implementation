...
class RRT:
    ...
    def get_new_point_in_ellipsoid(self, goal_bias, c_best):
        ...
        # Generate a random point in an ellipsoid
            ...
            x_center=np.array([[(self.start.row+self.goal.row)/2], [(self.start.col+self.goal.col)/2]])
            n_sg=((self.goal.row-self.start.row)**2+(self.goal.col-self.start.col)**2)**0.5
            a1=np.array([[(self.goal.row-self.start.row)/(n_sg**2)], [(self.goal.col-self.start.col)/(n_sg**2)]])
            l1=np.transpose(np.array([[1],[0]]))
            ...
            C=U@np.diag([1,np.linalg.det(U)*np.linalg.det(V)])@V_t
            
            x_br=np.random.normal(0,1)
            y_br=np.random.normal(0,1)

            x_ball=np.array([[x_br],[y_br]])
            
            L=np.array([[c_best/2, 0],[0,((c_best**2-c_min**2)**0.5)/2]])
